package com.example.mangaapp.models;

public class Relationships {
    private Links genres;
    private Links categories;
    private Links casting;
    private Links installments;
    private Links mapping;
    private Links reviews;
    private Links mediaRelationships;
    private Links characters;
    private Links staff;
    private Links productions;
    private Links quotes;
    private Links chapters;
    private Links mangaCharacters;
    private Links mangaStaff;

    public Links getGenres() {
        return genres;
    }

    public void setGenres(Links genres) {
        this.genres = genres;
    }

    public Links getCategories() {
        return categories;
    }

    public void setCategories(Links categories) {
        this.categories = categories;
    }

    public Links getCasting() {
        return casting;
    }

    public void setCasting(Links casting) {
        this.casting = casting;
    }

    public Links getInstallments() {
        return installments;
    }

    public void setInstallments(Links installments) {
        this.installments = installments;
    }

    public Links getMapping() {
        return mapping;
    }

    public void setMapping(Links mapping) {
        this.mapping = mapping;
    }

    public Links getReviewa() {
        return reviews;
    }

    public void setReviewa(Links reviewa) {
        this.reviews = reviewa;
    }

    public Links getMediaRelationships() {
        return mediaRelationships;
    }

    public void setMediaRelationships(Links mediaRelationships) {
        this.mediaRelationships = mediaRelationships;
    }

    public Links getCharacters() {
        return characters;
    }

    public void setCharacters(Links characters) {
        this.characters = characters;
    }

    public Links getStaff() {
        return staff;
    }

    public void setStaff(Links staff) {
        this.staff = staff;
    }

    public Links getProductions() {
        return productions;
    }

    public void setProductions(Links productions) {
        this.productions = productions;
    }

    public Links getQuotes() {
        return quotes;
    }

    public void setQuotes(Links quotes) {
        this.quotes = quotes;
    }

    public Links getChapters() {
        return chapters;
    }

    public void setChapters(Links chapters) {
        this.chapters = chapters;
    }

    public Links getMangaCharacters() {
        return mangaCharacters;
    }

    public void setMangaCharacters(Links mangaCharacters) {
        this.mangaCharacters = mangaCharacters;
    }

    public Links getMangaStaff() {
        return mangaStaff;
    }

    public void setMangaStaff(Links mangaStaff) {
        this.mangaStaff = mangaStaff;
    }
}

